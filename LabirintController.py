...
def OptimalWayLength(labirint, start, finish):
    q = deque()
    sizes = (len(labirint[0]), len(labirint))
...
def IsSpace(l, pos):
    okHorizontal = pos[0] >= 0 and pos[0] < len(l)
    okVertical = pos[1] >= 0 and pos[1] < len(l[0])
    if okHorizontal and okVertical and not l[pos[0]][pos[1]] == 1:
        return True
    return False
