...
def GenerateLabirint(length, height):
    ...
    def MakeWays(a):
        ...
        def GoTo(x, y):
            ...
            nonlocal stack
            nonlocal a
            IsSpace = LabirintController.IsSpace
            if not IsSpace(a, (2 * x + 1, 2 * y + 1)) or processed[x][y]:
                return False
            stack.append((x, y))
            processed[x][y] = True
            return True
        ...
        processed[0][0] = True
        while len(stack) > 0:
            d = [GoUp, GoDown, GoLeft, GoRight]
            random.shuffle(d)
            x = stack[-1][0]
            y = stack[-1][1]
            if d[0](x, y) or d[1](x, y) or d[2](x, y) or d[3](x, y):
                continue
            stack.pop()
        return a
